...
const labels_layer_1 = require("./labels.layer");
const helpers_1 = require("../helpers");
class StatusLayer extends labels_layer_1.LabelsLayer {
    page;
     ...
     */
    async sendAudioStatus(pathOrBase64, options) {
        let base64 = '';
        if (pathOrBase64.startsWith('data:')) {
            base64 = pathOrBase64;
        }
        ...
        if (!mimeInfo || !mimeInfo.includes('audio')) {
            const error = new Error('Not an audio, allowed formats mp3, wav, ogg, aac');
            Object.assign(error, {
                code: 'invalid_audio',
            });
            throw error;
    ...
    async sendReadStatus(chatId, statusId) {
        return await (0, helpers_1.evaluateAndReturn)(this.page, ({ chatId, statusId }) => {
